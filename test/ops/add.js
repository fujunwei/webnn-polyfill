...
describe('test add', function() {
  ...
  it('add constant and input', async function() {
    ...
    const b = builder.constant(
        {type: 'float32', dimensions: [3, 4, 5]}, {resource: await utils.createGPUBuffer(device, size, [
    ...
    graph.compute({'a': {resource: inputBuffer}}, {c: {resource: outputBuffer}});
    const expected = [
    ...
    const input1Buffer = await utils.createGPUBuffer(device, size, [
      1.08939514,  -1.5887482,  0.8545348,   0.20523034,  -0.41728342,
      1.01752,     0.19677015,  0.5398451,   0.56893295,  1.2511084,
      2.0092728,   1.0606714,   0.4893267,   0.09536829,  -2.3467007,
      2.4527607,   0.61307395,  -1.0799897,  -0.15071101, -0.48422927,
      -0.20479254, 0.32798728,  -0.37435308, -1.7116562,  1.6952512,
    ...
    graph.compute({'a': {resource: input1Buffer}}, {c: {resource: outputBuffer}});
    const expected1 = [
      0.5112086534500122, -2.0812354,  0.6382897,   0.07346585,  -0.93846387,
      2.9300475,   0.84765005,  1.2585825,   -1.7465117,  2.0591164,
      2.3115096,   1.2746171,   -0.16182771, 0.29538065,  -2.4271188,
      3.565543,    -0.90866506, -0.3550349,  -1.0703243,  -1.3159803,
      -1.6617002,  -0.21378079, -2.022044,   -1.5892761,  3.9158692,
    ...
    utils.checkValue(await utils.readbackGPUBuffer(device, size, outputBuffer), expected1);
  });
